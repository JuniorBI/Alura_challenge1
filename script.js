...
function criptografar(){
    if(input.value.length == 0){
        ...
        document.getElementById('decodificador').innerHTML = `<textarea readonly class="texto-desencriptado" id="mensagemDecript">${txt}</textarea>`;
...
function copiar(){
    document.querySelector('.texto-desencriptado').select();
    document.execCommand('copy');
    document.getElementById('msg').innerHTML = "Texto copiado para a área de transferência!";
}

let btnCriptografar = document.querySelector("button.botao-encriptar");
btnCriptografar.onclick = criptografar;

let btnDescriptografar = document.querySelector("button.botao-desencriptar");
btnDescriptografar.onclick = descriptografar;

let btnCopiar = document.querySelector("button.input-copiar");
